...
const createFollow=async(req,res)=>{
    ...
    if(!userExists){
        ...
        .then(response=>{
            ...
            return res.status(400).send("Error while making follow");
        })
...
const getNoFollowees=async(req,res)=>{
    ...
    const sql=`SELECT username,userId FROM user WHERE userId!=9 and userId not in(
        select followeeId from follow where followerId=9);`;
